...
-(id)initWithSelectTitle:(NSString *)title  NSDate:aDate  viewOfDelegate:(UIView *)view delegate:(id<MyDatePickerDelegate>)delegate{
    if (self=[super init]) {
        ...
        [ _datePicker setDate:aDate animated:YES];
        [self setTheTypeOfDatePicker:self.theTypeOfDatePicker];
        [self setIsBeforeTime:self.isBeforeTime];
...
-(void)dateConfirmClick{
    ...
    [self.delegate MyDatePickerDidConfirm:destDateString];

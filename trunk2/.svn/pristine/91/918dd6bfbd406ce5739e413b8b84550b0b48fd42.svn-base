...
- (BOOL)getCodeWithElements:(NSArray *)elements error:(NSError **)errPtr
{
    XMPPLogTrace();
    ...
    dispatch_block_t block = ^{ @autoreleasepool {
        ...
        NSXMLElement *queryElement = [NSXMLElement elementWithName:@"query" xmlns:@"jabber:iq:identifyingcode"];
        ...
        // Update state
        state = STATE_XMPP_GETIDENTFYINGCODE;
...
- (void)handleGETIdentifyingCode:(NSXMLElement *)response
{
    NSAssert(dispatch_get_specific(xmppQueueTag), @"Invoked on incorrect queue");
...
- (void)xmppParser:(XMPPParser *)sender didReadElement:(NSXMLElement *)element
{
    ...
    else if (state == STATE_XMPP_GETIDENTFYINGCODE)
    {
        // The iq response from our registration request
        [self handleGETIdentifyingCode:element];
    }

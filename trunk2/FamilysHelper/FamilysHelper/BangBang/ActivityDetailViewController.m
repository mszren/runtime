...
@interface ActivityDetailViewController () <UIScrollViewAccessibilityDelegate,
    UIWebViewDelegate,EGOImageViewDelegate,CXPhotoBrowserDataSource,CXPhotoBrowserDelegate>
...
- (void)viewDidLoad
{
    ...
    _activityModel = [AppCache loadCache:[NSString stringWithFormat:@"%@_%d",CACHE_BANG_BANGACTIVITYDETAIL,_activityId]];
    if (!_activityModel) {
    ...
    _detailRequest = [[TribeService shareInstance] getBangActivityDetail:_activityId
                                                               OnSuccess:^(DataModel* dataModel) {
                                                                   if (dataModel.code == 200) {
                                                                       ...
                                                                       [AppCache saveCache:[NSString stringWithFormat:@"%@_%d",CACHE_BANG_BANGACTIVITYDETAIL,_activityId] Data:_activityModel];
                                                                       [self initView];
                                                                   }
...
- (void)initView
{
    ...
    _images = [[EGOImageView alloc] initWithPlaceholderImage:[UIImage imageNamed:@"pic_default_320x215"]];
    _images.frame = CGRectMake(0, 0, SCREEN_WIDTH, 160);
    [_images setImageURL:[AppImageUtil getImageURL:_activityModel.image Height:_images.frame.size.height]];
    _images.tag = 1;
    _images.userInteractionEnabled = YES;
    _images.delegate = self;
    [_scrollView addSubview:_images];
    
    UITapGestureRecognizer *tapImage = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(onTapImage:)];
    [_images addGestureRecognizer:tapImage];
    ...
    [_images addSubview:statusButton];
...
- (void)showImgs:(NSUInteger)aIndex
{
     ...
     CXPhoto* photo = [[CXPhoto alloc] initWithURL:[NSURL URLWithString:getOriginalImage(_activityModel.image)]];

...
- (void)viewDidLoad {
    ...
    _tribe = [AppCache loadCache:[NSString stringWithFormat:@"%@_%d",CACHE_BANG_BANGDETAIL,_shopId]];
    if (!_tribe) {
   ...
   _hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    ...
    _request = [[TribeService shareInstance] getTribeInfoBy:self.shopId userID:[CurrentAccount currentAccount].uid OnSuccess:^(DataModel *dataModel) {
        ...
        if (dataModel.code == 200) {
            ...
            _tribe = dataModel.data;
            
            [AppCache saveCache:[NSString stringWithFormat:@"%@_%d",CACHE_BANG_BANGDETAIL,_shopId] Data:_tribe];

...
- (void)viewDidLoad {
    ...
    [self initView];
    _orderModel = [AppCache loadCache:[NSString stringWithFormat:@"%@_%@",CACHE_PERSON_RETURNORDER,_orderID]];
    if (!_orderModel) {
...
-(void)initData{
    ...
    _request = [[UserService shareInstance] getRefundInfoByMobile:_orderID OnSuccess:^(DataModel *dataModel) {
        if (dataModel.code == 200) {
            ...
            [AppCache saveCache:[NSString stringWithFormat:@"%@_%@",CACHE_PERSON_RETURNORDER,_orderID] Data:_orderModel];
            [self bindData];
        }
    }];
